...
final class Gs1DataMatrix
{
    ...
    {
        $gs1 = Gs1128::create($data);

        return new self($gs1->payload(), $gs1->elementString());
    }
    ...
    private function encodeAscii(): array
    {
        ...
        $i = 0;
        while ($i < $length) {
            if ($i + 1 < $length && ctype_digit($this->payload[$i]) && ctype_digit($this->payload[$i + 1])) {
                $codewords[] = 130 + (int) substr($this->payload, $i, 2);
                $i += 2;
                continue;
            }

            $ordinal = ord($this->payload[$i]);
            $codewords[] = $ordinal + 1;
            ++$i;
        }
    ...
    private function selectSymbol(int $dataCount): array
    {
        ...
        throw new InvalidCodeException('GS1 Data Matrix data is too long for a single-region symbol.');
    }
